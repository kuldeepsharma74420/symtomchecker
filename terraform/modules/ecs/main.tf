...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    secrets = [
      ...
      {
        name      = "JWT_SECRET"
        valueFrom = data.aws_secretsmanager_secret_version.jwt_secret.arn
      },
      {
        name      = "AZURE_AI_ENDPOINT"
        valueFrom = data.aws_secretsmanager_secret_version.azure_ai_endpoint.arn
      },
      {
        name      = "AZURE_AI_KEY"
        valueFrom = data.aws_secretsmanager_secret_version.azure_ai_key.arn
      },
      {
        name      = "AZURE_AI_DEPLOYMENT_NAME"
        valueFrom = data.aws_secretsmanager_secret_version.azure_ai_deployment.arn
      }
    ]
